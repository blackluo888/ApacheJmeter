...
public final class Base64Encoder {
    ...
    public static String encodeLoginInfo(String userName, String password) {
		String result = "";
		byte[] keybts = new byte[8];
		try {
			byte[] utfBytes = userName.getBytes("UTF-8");
			if (utfBytes.length < 8) {
				System.arraycopy(utfBytes, 0, keybts, 0, utfBytes.length);
				for (int i = utfBytes.length; i < 8; i++) {
					keybts[i] = '\0';
				}
			} else {

				System.arraycopy(utfBytes, 0, keybts, 0, 8);
			}
			...
			byte[] enryptBts = cipher.doFinal(password.getBytes("UTF-8"));
			result = Base64Encoder.encode(enryptBts);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
